...
{302.875000,-15.937500,-8.187500},
	...
	for ( int i = 0; i < m-1 ; i++ ){
		...
		// cout << linearaccel[0][i] << endl;
			...
			cumtrapz[i+1][0] = dt[0][i+1]*(linearaccel[i][0]+linearaccel[i+1][0]);
			cumtrapz[i+1][1] = dt[0][i+1]*(linearaccel[i][1]+linearaccel[i+1][1]);
			cumtrapz[i+1][2] = dt[0][i+1]*(linearaccel[i][2]+linearaccel[i+1][2]);
	...
	for ( int i = 0; i < m; i++ ){
		...
		velocitydata[i][0]=cumsum;
		velocitydata[i][1]=cumsum1;
		velocitydata[i][2]=cumsum2;

		// cout << cumsum2<< endl;
		// cout << velocitydata[i][2]<< endl;
...
//Implementing cumptrapz***********************************************
	...
	float totaldisplacement =3.28084 * sqrt( ((posdata[9][0]*posdata[9][0])+(posdata[9][1]*posdata[9][1])));
//***********************************************
cout<<endl;

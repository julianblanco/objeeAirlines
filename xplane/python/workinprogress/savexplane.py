		...
		with xpc.XPlaneConnect() as client:
				...
				while True:
						...
						if ( result ):
							...
							aa, Lat, Long, Alt, e, f, g, h, i =  result[3]
							...
							if count > 1:
								...
								listsend2 = ['<08',',',str(Lat)[:8],',',str(Long)[:9],',',str(Alt)[:5],'>\n']
							...
							if (count % 1)==0:
								...
								listsend = [str(Lat)[:8],',',str(Long)[:9],',0,0,0,0,',str(headg)[:5],',',str(gpsspeed)[:5],',0,',str(roll)[:4],',',str(pitch)[:4],',',str(roll_a),',',str(pitch_a),',0,0,0,0,',str(Alt)[:5],',0,',str(timesincestart),'\n']
								stringsend=''
								stringsend = ''.join("%s" % ''.join(map(str, x)) for x in listsend)
								# print stringsend
								# # print stringsend2

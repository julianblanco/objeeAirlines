...

{
    ...
    desiredspeed=speedcalc(headingError,distanceToTarget,maxspeed);

    steer = update_steering(headingError,GpsSpeed);

    speederror = desiredspeed - GpsSpeed;

    motorspeed = update_motors(speederror);
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    pwmSerial.println(steer);

    if(Fix)
    {
      pwmSerial.println(motorspeed);
    }
    else
    {
      pwmSerial.println(1500);
    }
    if (Fix)
    {
      notify_user();
...
void waypointing()
{
    ...
    if(distanceToTarget<3)
    {
        ...
        if (wpcounter<10)
        {
             wplat[0] = targetLat;
             wplong[0] = targetLong;
             wplat[1]=latgenerator(myLat,myLong,15,course  );
             wplong[1]=longgenerator(myLat,myLong,15,course );
             waypointNumber=1;
             course = createcourse(course,1);
             Serial.println(wpcounter);
        }
        else if(wpcounter<14)
        {
             wplat[0] = targetLat;
             wplong[0] = targetLong;
             wplat[1]=latgenerator(myLat,myLong,15,0 );
             wplong[1]=longgenerator(myLat,myLong,15,0);
             waypointNumber=1;
             Serial.println(wpcounter);
        } 
        else

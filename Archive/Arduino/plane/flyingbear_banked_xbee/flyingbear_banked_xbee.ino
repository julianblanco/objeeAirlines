...
double rollKp = 13, rollKi = .1, rollKd = 800000;

//Specify the links and initial tuning parameters
double pitchKp = 13, pitchKi = .1, pitchKd = 800000;

//Specify the links and initial tuning parameters
double yawKp = 6, yawKi = .1, yawKd = 800000;
...
void loop()
{
 ...
 // if (user_update)  UpdateUser();
  if (buffer_update) yawSetpoint= (double)newheading((int)yawSetpoint);
  // Other loop stuff
...
void ControlResponse(float yawTargetValue, float rollTargetValue, float pitchTargetValue, float yawInput, float rollInput, float pitchInput) {
  ...
  if (yawError >8) {rollTargetValue =-15; }

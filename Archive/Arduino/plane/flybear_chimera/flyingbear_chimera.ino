...
void UpdateServos() {
  ...
  leftServo.writeMicroseconds(rollServoOutput);
  rightServo.writeMicroseconds(rollServoOutput);
...
void ControlResponse(float yawTargetValue, float rollTargetValue, float pitchTargetValue, float yawInput, float rollInput, float pitchInput) {
  ...
  float bankResponse  = rollPID(rollError);
  float attackAngleResponse = pitchPID(pitchError);
  leftServoOutput = 1500 - abs(1500 -bankResponse)  +attackAngleResponse;
  rightServoOutput = bankResponse +attackAngleResponse;
  leftServoOutput=satuarationPoint(leftServoOutput,1800,1100);
  rightServoOutput=satuarationPoint(rightServoOutput,1800,1100);
...
float satuarationPoint(float input,float max,float min)
{

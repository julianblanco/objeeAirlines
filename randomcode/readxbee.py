...
fc= serial.Serial('/dev/ttyUSB0',baudrate=9600,   timeout = 3 )
data=''
result=''
count = 0
running = True
while ( running ):
	try:
		while True:
			...
			result=data.split(",")
			# print result
			myLat, myLong, targetLat,targetLong,targetHeading,trueHeading,GpsSpeed,wapointNumber,distanceToTarget,myAltitude,rollInput,rollTarget,pitchSetpoint,pitchInput,leftServo,rightservo, armed =result
			print(chr(27) + "[2J")
			print 'myLat        : ',myLat
			print 'myLong       : ',myLong
			...
			print 'rollTarget   :',rollTarget
			print 'rollInput    : ',rollInput
			print 'pitchTarget  :', pitchSetpoint
			print 'pitchInput   : ',pitchInput
			print 'armed        :',armed
			print 'leftServo    :',leftServo
			print 'rightServo   :', rightservo
